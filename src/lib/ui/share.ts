...
export function buildShareUrl(ctx: ShareContext, origin?: string): string {
  ...
  u.searchParams.set("node", ctx.nodeId === "tonga-kermadec" ? "tonga" : "cf");
  u.searchParams.set("window", ctx.windowKey);
  // X card cache bust / fresh scrape
...
export function buildShareText(ctx: ShareContext): string {
  const node =
    ctx.nodeId === "tonga-kermadec" ? "Tonga–Kermadec" : "Campi Flegrei";
  const bits: string[] = [`${node} monitor · SES`];
