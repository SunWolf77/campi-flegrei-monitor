...
const STORAGE_KEY = "ses-cf-epoch-log-v1";
  ...
  if (shouldOpen) {
    ...
    if (existing) {
      ...
      newEpoch = {
        ...
        trigger: C.rpamLabel,
        nodes: [nodeId, ...(nodeId === "tonga-kermadec" ? ["kermadec-islands", "hunga-tonga"] : [])],
...
export function buildEpochCsv(nodeHint?: string): string {
  ...
  lines.push(
    csvEscape(
      `# ses-cf-epoch-log csv · nodeHint=${nodeHint ?? ""} · elevatedEiiμ=${mem.priors.meanElevatedEii.toFixed(3)} · learned=${mem.learned.length}`,
    ),
  );
  return lines.join("\n");
}
