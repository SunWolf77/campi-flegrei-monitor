...
export function emptyCatalog(opts?: {
  ...
  const nodeId = opts?.nodeId ?? "campi-flegrei";
  ...
  return {
    events: [],
    provider: opts?.provider ?? (nodeId === "campi-flegrei" ? "gossip" : "usgs"),
    fetchedAt: Date.now(),
    sourceUrl: "",
    count: 0,
    window: {
    ...
    swarm: emptySwarm(),
    error: opts?.error,
    authority: nodeId === "campi-flegrei" ? "ingv-family" : "usgs-family",
    attempted: [],
  };
}
...
export function normalizeCatalog(
  ...
  return {
    ...
    authority:
      ...
      ((obj.nodeId as string) === "tonga-kermadec" ? "usgs-family" : "ingv-family"),
