...
export async function buildBoardHealth(opts: {
  ...
  const checks: HealthCheck[] = [
    { name: "app", status: "ok", detail: "campi-flegrei-monitor" },
    cronSecretCheck(),
  ];
  ...
  if (opts.deep) {
    ...
    checks.push(
      await probeUrl(
        "agency_gossip",
        `https://terremoti.ov.ingv.it/gossip/flegrei/csv/events_${year}.csv`,
      ),
      await probeUrl(
        "agency_ingv_fdsn",
        "https://webservices.ingv.it/fdsnws/event/1/query?format=text&minmag=3&limit=1&orderby=time",
      ),
      await probeUrl(
        "agency_usgs",
