...
export function MonitorApp({ initial }: Props) {
  ...
  const [nodeId, setNodeId] = useState<FocusNodeId>(() => {
    if (typeof window !== "undefined") {
      ...
      if (n === "ve" || n === "vesuvius" || n === "vesuvio") return "vesuvius";
      ...
      try {
        const result = await fetchCatalog({
          data: {
            ...
            maxDepthKm: isIngvGossipNode(nodeId)
              ? maxDepthKm == null
                ? 0
                : maxDepthKm
              : undefined,
            forceProvider: isIngvGossipNode(nodeId)
              ? "gossip"
  ...
  const selectNetworkNode = useCallback((id: FocusNodeId) => {
    ...
    else if (isIngvGossipNode(id) && maxDepthKm == null) setMaxDepthKm(8);
    setSelectedId(null);
    setTab("map");
  }, [maxDepthKm]);
  ...
  const filterActive =
    minMag > 0 || (isIngvGossipNode(nodeId) && maxDepthKm != null && maxDepthKm !== 8);
      ...
      <main className="mx-auto max-w-[1400px] min-w-0 overflow-x-hidden px-2 py-1.5 sm:px-4 sm:py-2">
        ...
        {tab === "supt" && (
          <div className="flex min-w-0 flex-col gap-3">
            <StressMapPanel events={events} node={node} swarm={swarm} height={mapHeightPx} spaceWeather={sw} schumannSnap={schumann} />
            <SuptDetective events={events} node={node} swarm={swarm} hideMap spaceWeather={sw} schumannSnap={schumann} />
          </div>
        )}
        ...
        {tab === "swarm" && (
          <div className={cn("grid gap-3", quiet && nodeId !== "tonga-kermadec" ? "" : "lg:grid-cols-[1.25fr_1fr]")}>
            <SwarmPanel swarm={swarm} events={events} nodeId={nodeId} onSelectCluster={focusCluster} onSelectEventId={setSelectedId} selectedEventId={selectedId} newCount={newSincePoll} />
          ...
          <div className="space-y-4">
            <section className="space-y-2">
              ...
              <div className="grid gap-3 lg:grid-cols-2">
                <SchumannPanel />
                {nodeId === "tonga-kermadec" ? <GeonetVolcanoPanel expanded /> : !quiet && <GeonetVolcanoPanel />}
              </div>
              {!quiet && <LaicBrief compact />}
            </section>
            <section className="space-y-2">
              <h3 className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground">Memory · export</h3>
              <EpochLogPanel nodeId={nodeId} continuum={continuum} swarm={swarm} schumann={schumann} density="full" enableLearn={false} />
            </section>
            {!quiet && (
              <details className="rounded-lg border border-border bg-card">
                ...
                <div className="grid gap-3 border-t border-border p-3 lg:grid-cols-2">
                  <PacificNodePanel />
                  <ObservationLinks nodeId={nodeId} />
                  <div className="lg:col-span-2"><NotionFramework /></div>
                </div>
              </details>
            )}
          </div>
        ...
        {tab === "catalog" && (
          <div className={cn("grid gap-3", quiet ? "" : "lg:grid-cols-[1.5fr_1fr]")}>
            <EventTable events={events} selectedId={selectedId} onSelect={(ev) => setSelectedId(ev.id)} />
            {!quiet && <ObservationLinks nodeId={nodeId} />}
          </div>
        )}
...
function Kpi({
  ...
  return (
    ...
    >
      ...
      {sub && compact && <div className="hidden truncate text-[9px] text-muted-foreground sm:block">{sub}</div>}
    </div>
  );
}
